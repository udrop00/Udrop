...
export default function Profile(){
 ...
 const[phone,setPhone]=useState(""); const[countryCode,setCountryCode]=useState("+1"); const[country,setCountry]=useState("United States");
 ...
 useEffect(()=>{apiMe().then(d=>{setName(d.user.name||"");setUsername(d.user.username||"");setEmail(d.user.email||"");setPhone(d.user.phone||"");setCountryCode(d.user.countryCode||"+1");setCountry(d.user.country||"United States");setStatus(d.user.status||"Active");setImage(d.user.profileImage||"")}).catch(()=>{})},[]);
 const save=async(e:FormEvent)=>{e.preventDefault();setSaved(false);setError("");const r=await apiFetch("/api/profile",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,profileImage:image,...(!username?{username}: {})})});const d=await r.json();if(!r.ok){setError(d.error||"Could not save");return}setUsername(d.user.username||username);setSaved(true);setTimeout(()=>setSaved(false),1800)};
 ...
 <form className="form-grid" onSubmit={save}>
  ...
  <label>Username<input value={username} readOnly placeholder="Choose your username" /></label>
  ...
  <div className="save-row" style={{gridColumn:"1/-1"}}><span>{saved?"✓ Changes saved":"Username, country, country code and phone number are fixed after registration."}</span><button className="btn" type="submit">Save changes</button></div>
